    ...
    if len(faces) > 0:
        ...
        if np.sum([roi_gray]) > 100:
            ...
            if max_pred > DETECTION_CONFIDENCE_THRESHOLD:
                return class_labels[preds.argmax()], max_pred, (x, y, w, h)

    return None, 0, None

def video_player():
    global current_speed, paused, exit_flag
    ...
    while not exit_flag:
        if not paused:
            ...
            cv2.putText(vid_frame, f"Speed: {current_speed:.1f}x", (20, 40), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.imshow('Video Player', vid_frame)

            with speed_lock:
                delay = int(frame_delay / current_speed)
...
try:
    ...
    while not exit_flag:
        ...
        if current_time - last_emotion_check >= EMOTION_CHECK_INTERVAL:
            emotion, confidence, face_coords = get_emotion(frame)
            last_emotion_check = current_time

            if emotion:
                recent_face_coords = face_coords
        ...
        cv2.putText(info_frame, f"Speed: {current_speed:.1f}x", (20, 40), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(info_frame, f"Next check in: {max(0, EMOTION_CHECK_INTERVAL - (current_time - last_emotion_check)):.1f}s", 
